...
import { Search, Filter, RefreshCw, Eye, FileText, Trash2 } from "lucide-react";
...
const DeleteBeneficiaryFilePage = () => {
  ...
  const handleDelete = (id: string) => {
    ...
    toast.success("تم حذف ملف المستفيد بنجاح");
  };
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {filteredBeneficiaries.length > 0 ? (
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  هل أنت متأكد من حذف ملف المستفيد "{beneficiary.name}"؟ هذا الإجراء لا يمكن التراجع عنه.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
            ...
            <EmptyState
              icon={FileText}
              message="لم يتم العثور على أي مستفيدين مطابقين للبحش"
            />
          )}

          {/* Pagination Info */}
          <div className="flex items-center justify-between mt-4 text-sm text-muted-foreground">
            <span>عرض 1 إلى {filteredBeneficiaries.length} من {beneficiaries.length} سجل</span>
          </div>
        </CardContent>
      </Card>
    </InnerPageLayout>
